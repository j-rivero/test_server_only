...
| `no-gui-deps` | `Dockerfile.no-gui-deps` | Comprehensive dependency audit of `gz-sim10-server`. Scans all installed packages against a broad set of GUI patterns (Qt, OGRE, X11, Mesa, Wayland, XCB, `libgz-gui`, `libgz-sim*-gui`). On any match, traces reverse dependencies to identify the offending chain and fails the build. Also verifies specific Gazebo GUI packages are absent, checks installation size, and lists physics engine packages. |
